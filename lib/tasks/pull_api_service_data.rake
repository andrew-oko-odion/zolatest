namespace :pull_api_service_data do
  ...
  task pull_data: :environment do
    ...
    country.each do |value|
      countries_data << {name: value[:name], population_density: value[:population], country_code: value[:cioc], exchange_rate: value[:exchange_rate], currency_code: value[:currencies][0][:code]}
    end
   # Save to db 
